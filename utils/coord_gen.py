...
def coords_gen(coord_path, coord_format, base_dir):
    ...
    for dir in coords_list:
        ...
        try:
            ...
            print('np32:',data.shape)

        np.savetxt(os.path.join(base_dir, "coords", dir),
                   data, delimiter='\t', newline="\n", fmt="%s")#把生成的numpy数组保存行是粒子个数，列是粒子坐标
        num_all.append(data.shape[0])#每个coords粒子数量
...
if __name__=='__main__':
    coord_path='D:\desktop\study\picker_pack\DeepETPicker-main\DeepETPicker-main\data_set'
    coord_format='coords'
    stdout=None
    coords_gen(coord_path, coord_format, base_dir)