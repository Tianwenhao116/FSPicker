...
class UNetExperiment(pl.LightningModule):
    def __init__(self, args):
        ...
        self.partical_volume = 4 / 3 * np.pi * (self.val_cfg["label_diameter"] / 2) ** 3#获取颗粒体积（球形）
        self.args = args
    ...
    def train_dataloader(self):
        ...
        train_dataset = Dataset_ClsBased(mode=args.train_mode,
                                         ...
                                         num_class=args.num_classes,#15
                                         random_num=args.random_num,#0
                                         use_bg=args.use_bg,#true
                                         data_split=args.data_split,#0,7,7,8,7,8
                                         use_paf=args.use_paf,#false
                                         cfg=self.train_cfg,
                                         args=args)
    ...
    def val_dataloader(self):#加载测试数据集
        args = self.args
        ...
        if 'test' in args.test_mode:
            ...
            self.gt_coords = val_dataset.gt_coords#在test模式下v
...
def train_func(args, stdout=None):
    ...
    logger_name = "{}_{}_BlockSize{}_{}Loss_MaxEpoch{}_bs{}_lr{}_IP{}_bg{}_Softmax{}_{}_{}_TN{}".format(
        model.train_cfg["dset_name"], args.network, args.block_size, args.loss_func_seg, args.max_epoch,
        args.batch_size,
        args.learning_rate,
        int(args.use_IP), int(args.use_bg),
